...
export type FastParseWithCount = (str: string) => [number | null, number];
...
function buildCore(language: Language, withMatchCount: boolean): {
	fn(ROOT: Uint8Array, BOUND: Uint8Array, str: string): number | null | [number | null, number];
	rootArr: Uint8Array;
...
export function buildFastParse(language: Language, withMatchCount = false): FastParseFunction | FastParseWithCount {
	...
	return fn.bind(null, rootArr, boundaryArr) as FastParseFunction | FastParseWithCount;
}

/** @internal For JIT inspection only — exposes the raw generated fn before .bind() */
export function buildFastParseRaw(language: Language): {
	fn(ROOT: Uint8Array, BOUND: Uint8Array, str: string): number | null;
	rootArr: Uint8Array;
	boundaryArr: Uint8Array;
} {
	const { fn, rootArr, boundaryArr } = buildCore(language, false);
