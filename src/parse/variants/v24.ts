...
function generateMatchCode(node: TrieNode, indent: string): string {
	...
	if (node.multiplier !== null)
		code += `${indent}if(i>=len||c>=128||!BOUND[c]){v+=pv*${node.multiplier};mc++;return true}\n`;
	...
	if (node.children.size === 1) {
		...
		for (let k = 0; k < chain.length; k++) {
			const [ lo, hi ] = chain[k]!;
			if (lo === hi)
				code += `${indent}if(s.charCodeAt(i+${k})!==${lo})return false;\n`;
			else
				code += `${indent}{const _c=s.charCodeAt(i+${k});if(_c!==${lo}&&_c!==${hi})return false;}\n`;
		}
