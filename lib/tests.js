...
describe('inputs', () => {
	it('should not throw an error', () => {
		expect(() => {
			...
			ms('1m');
			ms('53 milliseconds');
			ms('53 milliseconds', 'all');
			ms('53 milliseconds', 'es');
			ms('53 milliseconds', 'en');
			ms('53 milliseconds', ['es', 'en']);
			ms(500);
			ms(500, { long: true });
			ms(500, { long: false });
			ms(500, { long: true, length: 8 });
			ms(1244122312344, { long: true, length: 8, format: 'YMoWDHMSMs', language: 'en' });
		}).toNotThrowError();
	});
	...
	it('should throw an error', () => {
		expect(() => ms(1200, { long: '' })).toThrowError();
		expect(() => ms(1200, { length: false })).toThrowError();
		expect(() => ms(1200, { format: false })).toThrowError();
		expect(() => ms(1200, { language: false })).toThrowError();

		expect(() => ms(1200, { length: 0 })).toThrowError();
		expect(() => ms(1200, { length: 9 })).toThrowError();
		expect(() => ms(1200, { format: 'A' })).toThrowError();
		expect(() => ms(1200, { language: 'ññ' })).toThrowError();

		expect(() => ms('1 day', 'ññ')).toThrowError();
		expect(() => ms('1 day', ['ññ'])).toThrowError();
		expect(() => ms('1 day', ['all'])).toThrowError();
	});
});
